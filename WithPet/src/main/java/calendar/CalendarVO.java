...
public class CalendarVO {
	...
	public Map<String, Integer> today_info(CalendarVO vo) {
		...
		//위에서 search_month에 들어가는 값에 +1 해준것을 없애주기
	...
	private Map<String, Integer> before_after_calendar(int search_year, int search_month) {
		...
		//이때 이때 berfore_month가 0보다 작다는 것은 전년도 11월을 의미하기 때문에 before_month를 11로 바꿔준다.
		if(before_month < 0) {
			before_month = 11;	//전년도 11월로 바꿔준다.
			before_year=search_year-1;	//전년도로 바꿔준다.
		}//if
		
		//위에 멤버변수 선언때 before_month = search_month + 1을 해줬는데
		//이때 이때 berfore_month가 11보다 크다는 것은 다음해를 의미하기 때문에 after_month를 0로 바꿔준다.
		if(after_month > 11) {
			after_month = 0;
